...
The server exposes 30+ tools across six areas.
...
### Workspace health and analytics

The same read-only health and analytics tools the [workspace agent](/agents/workspace-agent) uses, answered for the key's space:

| Tool | What it does |
| --- | --- |
| `get_org_score` | Org-health score out of 100 with its breakdown and trend |
| `list_integrations` | The org's API surface — inbound endpoints it serves and outbound systems it calls |
| `get_coverage` | Documented versus total components per connected org, with the largest gaps |
| `get_freshness` | Doc freshness scores, stalest first |
| `get_analytics_model` | The analytics semantic model — cubes, measures, dimensions (needs an analytics source) |
| `query_analytics` | A governed analytics query over the org's records (needs an analytics source) |
