...
Reach the agent where your team already works: connect **Slack** so agents can respond in your channels (see [Slack integration](/integrations/slack)), post a card to **Microsoft Teams** when a background run finishes via an incoming webhook, and mint **public API keys** for driving runs from your own tools.
