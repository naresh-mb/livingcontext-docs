...
When the Live file list is open, the Welcome page prepares in the background
so it is ready sooner when you select it. Opening a Live page also prepares the
next Markdown page in reading order, making **Next** navigation quicker.

The space selector is available when the page opens, alongside the file tree.
Use it to switch repositories. The next
repository opens as your selection is saved, and recently visited repositories
reuse their file listing. If saving the selection fails, the viewer returns to
the last saved repository and shows an error. Live pages and drafts
keep separate content. Creating, renaming, or deleting pages refreshes the
listing automatically.

Repositories in the same space can show their file list while their drafts
finish loading. The draft picker shows progress and offers a refresh if that
request fails.

### Images and screenshots

Images, captured screenshots, and captured clips appear in a slim abstract frame
with rounded corners and a soft shadow. Each image starts with a varied background
that stays the same when you reopen the page. Captions sit below the frame.

To choose a background in a document you can edit, hover over the image and select
**Background**. Choose **Sequoia** or **Ventura**, or use **Shuffle background** for
a different choice. The selected background saves with the image. On touch screens,
the Background button stays visible.
...
Double-click an ordinary image to open a larger preview with zoom controls. Images
from private repositories use the same access as the page.

To change an image in a document you can edit, select it and choose **Edit image**.
Hover over the image for zoom controls, or open **Editing tools** to crop and
annotate. Captured screenshots already have these controls. Your adjustments
save with the document and remain when you reopen it; the original image stays
available for reframing. Readers without edit rights see the saved result.
