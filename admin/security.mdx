...
<CardGroup cols={2}>
  <Card title="Two-factor authentication" icon="shield-check">
    Every member can enable TOTP two-factor authentication (any authenticator app) with one-time backup codes, from **Settings → Security**. Sign-in for enrolled accounts requires the second factor. Ten failed verification attempts temporarily pause second-factor verification for 15 minutes.
  </Card>
...
Passwords follow a modern (NIST-aligned) policy — a 12-character minimum with a common-password deny-list, enforced server-side — and the sign-in endpoints are rate-limited against brute force.
...
Invitation recipients must verify the invited email address before viewing, accepting, or declining an invitation. The invitation page offers a verification email and returns them to the invitation afterward. This protection applies to every organization.
...
| **IP allowlist** | Restrict organization access to specific networks (IPv4/IPv6 addresses or CIDR blocks). Enforced on every path in — web sessions, the capture extension, and MCP keys. Saving a list that would lock _you_ out is rejected. |
...
When agents ground documentation against your live Salesforce org, they work from **metadata, not record contents**. For example, field utilization analysis is count-only — the agent learns _how many_ records populate a field, never _what's in_ them.
...
Being transparent about what's _not_ built yet:
...
| SAML | Planned for required SAML-only identity providers. OIDC SSO supports Okta, Entra, Google Workspace, Auth0 and other modern identity providers today. |
